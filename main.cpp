...
int main()
{
	...
	cout << "Base case empty 999,999 loop" << endl;
	Stopwatch base;
	for (int i = 0; i != 999999; ++i)
	{
	}
	base.log();
	double base_secs = base.seconds_elapsed();

	cout << endl;

	cout << "Initialize Decimals from 999,999 strings vs base" << endl;
	Stopwatch string_init_timer;
	...
	cout << (string_init_timer.seconds_elapsed() - base_secs) << " seconds."
	     << endl;

	cout << endl;

	
	cout << "999,999 lexical casts vs base" << endl;
	Stopwatch lex_cast_timer;
	for (int i = 0; i != 333333; ++i)
	{
		long x = lexical_cast<long>("-901234");
		long y = lexical_cast<long>("90234");
		long z = lexical_cast<long>("-132");
	}
	cout << (lex_cast_timer.seconds_elapsed() - base_secs) << " seconds."
	     << endl;
